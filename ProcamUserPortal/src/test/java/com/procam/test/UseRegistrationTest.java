...
public class UseRegistrationTest extends BaseClass {
    ...
    private Map<String, String> eventDashboardData;
    private Map<String, String> personalData;
    private Map<String, String> eventCriteriaData;
    private Map<String, String> merchandiseData;
    private Map<String, String> gstData;
    private Map<String, String> paymentsData;
	...
	public void setup() {
		...
		// Load all data once
        loginData = ExcelUtils.getTestData(inputFile, "Login", 1);
        eventDashboardData = ExcelUtils.getTestData(inputFile, "EventDashboard", 1);
        personalData = ExcelUtils.getTestData(inputFile, "Personal", 1);
        eventCriteriaData = ExcelUtils.getTestData(inputFile, "EventCriteria", 1);
        merchandiseData = ExcelUtils.getTestData(inputFile, "Merchandise", 1);
        gstData = ExcelUtils.getTestData(inputFile, "GSTData", 1);
        paymentsData = ExcelUtils.getTestData(inputFile, "Payments", 1);
	}

	/*
	 * @Test(dataProvider = "fullFlowData") public void verifyLogin(Map<String,
	 * String> loginData, Map<String, String> eventDashboardData, Map<String,
	 * String> personalData, Map<String, String> eventCriteriaData, Map<String,
	 * String> MerchandiseData, Map<String, String> gstData, Map<String, String>
	 * PaymentsData) throws InterruptedException {
	 * test=ExtentReport.createTest("Login Test");
	 * 
	 * loginPage = new LoginPage(); eventDashboardPage =
	 * loginPage.loginByEmail(loginData); discountApplyPage =
	 * eventDashboardPage.selectEvent(eventDashboardData); //
	 * personalDetailsPage=discountApplyPage.withDiscountCode();
	 * 
	 * //discountApplyPage.readMore(); personalDetailsPage =
	 * discountApplyPage.withoutDiscountCode();
	 * 
	 * eventCriteriaPage = personalDetailsPage.enterDetails(personalData);
	 * 
	 * //eventCriteriaPage.eventLinkPage(); merchandiseDetailsPage =
	 * eventCriteriaPage.enterEventDetails(eventCriteriaData);
	 * 
	 * orderSummaryPage =
	 * merchandiseDetailsPage.enterMerchandiseDetails(MerchandiseData);
	 * 
	...
	public void testEventDashboard() {
		ExtentReport.createTest("Select Event Test");
		discountApplyPage=eventDashboardPage.selectEvent(eventDashboardData);
		
	}
	
	@Test(dependsOnMethods = "testEventDashboard")
	public void testPersonalDetails() throws InterruptedException {
		ExtentReport.createTest("Personal Details Test");
	...
	public void testMerchandise() throws InterruptedException {
		ExtentReport.createTest("Merchandise Test");
		orderSummaryPage=merchandiseDetailsPage.enterMerchandiseDetails(merchandiseData);
	}
	
	@Test(dependsOnMethods = "testMerchandise")
	public void testGSTDetails() {
		ExtentReport.createTest("GST Details Test");
		paymentsOptionPage=orderSummaryPage.enterGstDetails(gstData);
	}
